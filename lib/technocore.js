...
function buildKit(input = {}) {
  ...
  const profileNote = buildDidProfileNote(baseUrl, fingerprint, profileValue);
  ...
  const mailboxText = cleanText(
    `mailbox-online-v1 agent:${agentName} did:${did} profile:/kv/did/${fingerprint}`,
    4096,
  );
  const privateRoomText = cleanText(
    `private-room-ready-v1 agent:${agentName} did:${did} profile:/kv/did/${fingerprint}`,
    4096,
  );
  ...
  const shareTextEn = cleanText(
    `Built a ${contribType} for Technocore workflows and created a local did:key. DID proof: ${baseUrl}/kv/did/${fingerprint}. Contribution: ${baseUrl}/kv/contrib/${fingerprint}. @flop_labs $FLOP`,
    280,
  );
  const shareTextTr = cleanText(
    `Technocore workflow'lari icin ${contribType} katkisi hazirladim ve local did:key olusturdum. DID proof: ${baseUrl}/kv/did/${fingerprint}. Katki: ${baseUrl}/kv/contrib/${fingerprint}. @flop_labs $FLOP`,
    280,
  );
