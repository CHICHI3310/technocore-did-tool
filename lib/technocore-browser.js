...
(() => {
  ...
  async function buildSignedLobbyMessage(privateKeyJwk, nonce) {
    const did = didFromPublicJwk(privateKeyJwk);
    if (did !== SIGNED_LOBBY_DID) throw new Error("Imported DID does not match the required lobby DID.");
    if (!/^\d{1,19}$/.test(String(nonce))) throw new Error("Lobby nonce must be 1-19 digits.");
    const cryptoKey = await crypto.subtle.importKey("jwk", privateKeyJwk, { name: "Ed25519" }, false, ["sign"]);
    const canonical = `lobby|${nonce}|${SIGNED_LOBBY_TEXT}`;
    const signature = await crypto.subtle.sign("Ed25519", cryptoKey, new TextEncoder().encode(canonical));
    return {
      did,
      sig: bytesToBase64url(new Uint8Array(signature)),
      nonce: String(nonce),
      text: SIGNED_LOBBY_TEXT,
    };
  }

  async function identityFromPrivateKey(privateKeyJwk) {
    const did = didFromPublicJwk(privateKeyJwk);
    return { did, fingerprint: await fingerprintOfDid(did) };
  }

  async function postSignedLobbyMessage(message, fetchImpl = fetch) {
    if (!message || message.did !== SIGNED_LOBBY_DID || message.text !== SIGNED_LOBBY_TEXT) {
      throw new Error("Lobby message does not match the required DID or text.");
    }
    return fetchImpl("https://technocore.chat/r/lobby", {
      method: "POST",
  ...
  window.TechnocoreBrowser = {
    WARNING,
    SIGNED_LOBBY_DID,
    SIGNED_LOBBY_TEXT,
    buildKit,
    buildSignedLobbyMessage,
    createDid,
    didFromPublicJwk,
    identityFromPrivateKey,
    postSignedLobbyMessage,
  };
})();
